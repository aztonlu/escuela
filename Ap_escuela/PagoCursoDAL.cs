        ...
        public void Buscarcarga(int dni, DataGridView dgv)
        {
            using (SqlConnection conexion = BDComun.ObtnerCOnexion())
                ...
                try
                {
                    ...
                    da = new SqlDataAdapter(string.Format("Select tmatricula.numerogrupo AS Grupo, tmatricula.idcurso AS Codigo_Curso, tcurso.nombrecurso AS Nombre_curso, tcurso.monto AS Costo_Curso from tmatricula inner join talumno on tmatricula.dni = talumno.dni inner join tcurso on tmatricula.idcurso = tcurso.idcurso where talumno.dni like '%{0}%'", dni), conexion);
                    dt = new DataTable();
                    da.Fill(dt);
                    dgv.DataSource = dt;
                }
